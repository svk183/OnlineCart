...
function updateLinting(options) {
    return schematics_1.chain([
        ...
        workspace_1.updateJsonInTree(`${options.appProjectRoot}/tslint.json`, json => {
            json.extends = `${workspace_1.offsetFromRoot(options.appProjectRoot)}tslint.json`;
            return json;
        })
    ]);
}
function addSchematicFiles(appProjectRoot, options) {
    return schematics_1.chain([
        ...
        schematics_1.mergeWith(schematics_1.apply(schematics_1.url('./files'), [
            schematics_1.template(Object.assign({}, options, { offsetFromRoot: workspace_1.offsetFromRoot(options.appProjectRoot) })),
        ...
        return schematics_1.chain([
            workspace_1.updateJsonInTree(workspace_1.getWorkspacePath(host), json => {
                ...
                fixedProject.architect.lint.options.tsConfig = fixedProject.architect.lint.options.tsConfig.filter(path => path !==
                    ...
                    path !==
                        core_1.join(core_1.normalize(options.appProjectRoot), 'e2e/tsconfig.json'));
                fixedProject.architect.lint.options.exclude.push('!' + core_1.join(core_1.normalize(options.appProjectRoot), '**'));
            ...
            workspace_1.updateJsonInTree(`${options.appProjectRoot}/tsconfig.app.json`, json => {
                return Object.assign({}, json, { extends: `./tsconfig.json`, compilerOptions: Object.assign({}, json.compilerOptions, { outDir: `${workspace_1.offsetFromRoot(options.appProjectRoot)}dist/out-tsc` }), exclude: options.enableIvy
                        ? undefined
                        : options.unitTestRunner === 'jest'
                            ? ['src/test-setup.ts', '**/*.spec.ts']
                            : ['src/test.ts', '**/*.spec.ts'], include: [options.enableIvy ? 'src/**/*.d.ts' : '**/*.ts'] });
            }),
            host => {
            ...
            workspace_1.updateJsonInTree(`/nx.json`, json => {
                const resultJson = Object.assign({}, json, { projects: Object.assign({}, json.projects, { [options.name]: { tags: options.parsedTags } }) });
...
function updateE2eProject(options) {
    return (host) => {
        ...
        host.overwrite(spec, content.replace('my-app app is running!', `Welcome to ${options.name}!`));
        return schematics_1.chain([
            workspace_1.updateJsonInTree(workspace_1.getWorkspacePath(host), json => {
                const project = {
                    ...
                    architect: {
                        ...
                        lint: {
                            ...
                            options: {
                                ...
                                exclude: [
                                    '**/node_modules/**',
                                    '!' + core_1.join(core_1.normalize(options.e2eProjectRoot), '**')
                                ]
                            }
                        }
                    }
            ...
            workspace_1.updateJsonInTree(`${options.e2eProjectRoot}/tsconfig.e2e.json`, json => {
                return Object.assign({}, json, { extends: `./tsconfig.json`, compilerOptions: Object.assign({}, json.compilerOptions, { outDir: `${workspace_1.offsetFromRoot(options.e2eProjectRoot)}dist/out-tsc` }) });
            })
        ]);
    };
}
...
function default_1(schema) {
    return (host, context) => {
        ...
        const e2eProjectRoot = workspaceJson.newProjectRoot
            ...
            : `${options.name}/e2e`;
        return schematics_1.chain([
            init_1.default(Object.assign({}, options, { skipFormat: true })),
...
function normalizeOptions(host, options) {
    ...
    let e2eProjectName = `${workspace_1.toFileName(options.name)}-e2e`;
    ...
    return Object.assign({}, options, { prefix: options.prefix ? options.prefix : defaultPrefix, name: appProjectName, appProjectRoot,
        e2eProjectRoot,
        e2eProjectName,
        parsedTags });
}
