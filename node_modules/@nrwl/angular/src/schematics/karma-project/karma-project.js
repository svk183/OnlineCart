...
function generateFiles(options) {
    return (host, context) => {
        ...
        return schematics_1.mergeWith(schematics_1.apply(schematics_1.url('./files'), [
            schematics_1.template(Object.assign({ tmpl: '' }, options, { projectRoot: projectConfig.root, isLibrary: projectConfig.projectType === 'library', offsetFromRoot: workspace_1.offsetFromRoot(projectConfig.root) })),
...
function updateTsConfig(options) {
    return (host) => {
        ...
        return workspace_1.updateJsonInTree(core_1.join(projectConfig.root, 'tsconfig.json'), json => {
            return Object.assign({}, json, { compilerOptions: Object.assign({}, json.compilerOptions, { types: Array.from(new Set([...(json.compilerOptions.types || []), 'jasmine'])) }) });
        });
    };
}
function updateTsSpecConfig(options) {
    return (host) => {
        ...
        return workspace_1.updateJsonInTree(core_1.join(projectConfig.root, 'tsconfig.spec.json'), json => {
            return Object.assign({}, json, { files: [...json.files, ...extraFiles] });
        });
    };
}
function updateworkspaceJson(options) {
    return workspace_1.updateWorkspaceInTree(json => {
        ...
        if (projectConfig.projectType === 'application') {
            projectConfig.architect.test.options = Object.assign({}, projectConfig.architect.test.options, { polyfills: core_1.join(core_1.normalize(projectConfig.sourceRoot), 'polyfills.ts'), styles: [], scripts: [], assets: [] });
        }
