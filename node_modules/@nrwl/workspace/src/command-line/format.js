...
function format(command, args) {
    ...
    try {
        ...
        output_1.output.error({
            title: e.message,
            bodyLines: [
                `Pass the SHA range: ${output_1.output.bold(`npm run format:${command} -- SHA1 SHA2`)}`,
                '',
    ...
    const chunkList = chunkify(patterns, 70);
    switch (command) {
        case 'write':
...
function getPatternsFromApps(affectedFiles) {
    const roots = shared_1.getProjectRoots(touched_1.getTouchedProjects(affectedFiles));
