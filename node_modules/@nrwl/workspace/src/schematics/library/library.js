...
function createFiles(options) {
    return schematics_1.mergeWith(schematics_1.apply(schematics_1.url(`./files/lib`), [
        schematics_1.template(Object.assign({}, options, workspace_3.names(options.name), { tmpl: '', offsetFromRoot: workspace_5.offsetFromRoot(options.projectRoot) })),
        schematics_1.move(options.projectRoot)
    ]));
}
function updateNxJson(options) {
    return workspace_2.updateJsonInTree('nx.json', json => {
...
function normalizeOptions(options) {
    ...
    const parsedTags = options.tags
        ...
        : [];
    return Object.assign({}, options, { fileName, name: projectName, projectRoot,
        projectDirectory,
        parsedTags });
}
